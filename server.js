...
app.get("/phone-numbers/:numberId", (req, res, next) => {
    try {
        const foundNumber = phoneNumbers.find((number) => req.params.numberId === number.id);
        if (foundNumber) {
...
app.post("/phone-numbers", (req, res, next) => {
    try {
        ...
        throw new Error("Bad request. You have to send an object with the number and name fields");
    }
    catch (error) {
        next(error);
    }
...
app.delete("/phone-numbers/:numberId", (req, res, next) => {
    try {
        const foundNumber = phoneNumbers.find((number) => req.params.numberId === number.id);
        if (foundNumber) {
            const newItems = phoneNumbers.filter((number) => number.id !== req.params.numberId);
...
app.patch("/phone-numbers/:numberId", (req, res, next) => {
    try {
        ...
        throw new Error("Bad request. You have to send an object with a number or name field. Or number not found");
    }
    catch (error) {
        next(error);
    }
